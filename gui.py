...
class CursorProButton(tk.Frame):
    ...
    def __init__(self, parent, text, command, style="primary", **kwargs):
        ...
        # Button styles - 修复 M2 macOS 兼容性
        if style == "primary":
            self.normal_bg = '#007AFF'  # 使用 macOS 系统蓝色
            self.normal_fg = '#FFFFFF'
            self.hover_bg = '#0056CC'
            self.hover_fg = '#FFFFFF'
        elif style == "secondary":
            self.normal_bg = '#34C759'  # 使用 macOS 系统绿色
            self.normal_fg = '#FFFFFF'
            self.hover_bg = '#28A745'
            self.hover_fg = '#FFFFFF'
        elif style == "warning":
            self.normal_bg = '#FF9500'  # 使用 macOS 系统橙色
            self.normal_fg = '#FFFFFF'
            self.hover_bg = '#E6850E'
            self.hover_fg = '#FFFFFF'
        else:
            self.normal_bg = '#E5E5E7'  # 使用 macOS 系统灰色
            self.normal_fg = '#000000'
            self.hover_bg = '#D1D1D6'
        ...
        self.canvas = tk.Canvas(self, height=55, highlightthickness=0,
                               bg='#F2F2F7', cursor='hand2')  # 使用 macOS 系统背景色
    ...
    def _get_system_font(self):
        """获取适合当前系统的字体 - 修复 M2 macOS 兼容性"""
        system = platform.system().lower()
        if system == "darwin":  # macOS
            # 在 macOS 上使用系统字体，特别针对 M2 芯片优化
    ...
    def _draw_button(self):
        ...
        # Draw text - 修复 M2 macOS 字体问题
        # 使用 macOS 系统字体，确保在 M2 芯片上正确显示
        font_family = self._get_system_font()
        self.canvas.create_text(width//2, height//2, text=self.text,
                               fill=text_color, font=(font_family, 13, 'normal'))
...
class ModernDialog:
    ...
    def __init__(self, parent, title, message, dialog_type="info"):
        ...
        title_label = tk.Label(main_frame, text=title,
                              ...
                              fg='#1C1C1E', bg='#f5f5f5')  # 使用 macOS 系统文字颜色
        title_label.pack(pady=(0, 15))

        # Message
        message_label = tk.Label(main_frame, text=message,
                                font=(system_font, 10),
                                fg='#3C3C43', bg='#f5f5f5',  # 使用 macOS 系统文字颜色
        ...
        if dialog_type == "warning":
            ...
            # Cancel button on the left - 修复字体问题
            ...
            def on_confirm_enter(_):
                yes_btn.config(bg='#0056CC')  # 使用 macOS 系统蓝色
            def on_confirm_leave(_):
                yes_btn.config(bg='#007AFF')
            yes_btn.bind('<Enter>', on_confirm_enter)
            yes_btn.bind('<Leave>', on_confirm_leave)

        else:
            # OK button for info dialogs - 修复字体问题
            ...
            def on_ok_enter(_):
                ok_btn.config(bg='#0056CC')  # 使用 macOS 系统蓝色
            def on_ok_leave(_):
                ok_btn.config(bg='#007AFF')
...
class AugmentToolsGUI:
    def __init__(self, root):
        ...
        # Set window style - 修复 M2 macOS 兼容性
        self.root.configure(bg='#F2F2F7')  # 使用 macOS 系统背景色
    ...
    def setup_gui(self):
        ...
        # Main container - 修复 M2 macOS 兼容性
        main_frame = tk.Frame(self.root, bg='#F2F2F7')  # 使用 macOS 系统背景色
        main_frame.pack(fill='both', expand=True, padx=30, pady=20)

        # Logo/Icon area - 修复 M2 macOS 兼容性
        icon_frame = tk.Frame(main_frame, bg='#F2F2F7', height=100)
        icon_frame.pack(fill='x', pady=(0, 20))

        # Create gradient title using canvas that fills the frame width
        title_canvas = tk.Canvas(icon_frame, height=60, bg='#F2F2F7',
        ...
        welcome_label = tk.Label(icon_frame, text="欢迎使用",
                                font=(system_font, 12),
                                fg='#3C3C43', bg='#F2F2F7')  # 使用 macOS 系统文字颜色
        welcome_label.pack(pady=(0, 25))

        # IDE Selection Section - 修复 M2 macOS 兼容性
        ide_section = tk.Frame(main_frame, bg='#F2F2F7')
        ide_section.pack(fill='x', pady=(0, 20))

        system_font = self.get_system_font()
        ide_label = tk.Label(ide_section, text="选择 IDE:",
                            font=(system_font, 11, 'bold'),
                            fg='#1C1C1E', bg='#F2F2F7')  # 使用 macOS 系统文字颜色
        ...
        self.ide_combo = ttk.Combobox(ide_select_frame,
                                     ...
                                     state="readonly",
                                     font=(system_font, 10),  # 修复字体问题
                                     style='Custom.TCombobox')
        self.ide_combo.pack(fill='x', padx=10, pady=8)

        # Buttons container - 修复 M2 macOS 兼容性
        buttons_frame = tk.Frame(main_frame, bg='#F2F2F7')
        ...
        # Version info at bottom - 修复 M2 macOS 兼容性
        version_frame = tk.Frame(main_frame, bg='#F2F2F7')
        version_frame.pack(fill='x', pady=(40, 20))  # More space at bottom

        system_font = self.get_system_font()
        version_label = tk.Label(version_frame, text="v0.0.3 - 多IDE支持版本 (M2 macOS 优化)",
                                font=(system_font, 12),
                                fg='#8E8E93', bg='#F2F2F7')  # 使用 macOS 系统次要文字颜色
        version_label.pack()
        ...
        self.status_label = tk.Label(version_frame, textvariable=self.status_var,
                                    font=(system_font, 10),
                                    fg='#34C759', bg='#F2F2F7')  # 使用 macOS 系统绿色
